...
from .services import save_summoner_info, save_matches_stats
...
def get_summoner(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                save_summoner_info(summoner_name, summoner_tag, summoner_region, api_key)
                return JsonResponse({'redirectUrl': f'/data_visualization/{summoner_name}'})
            except ValidationError as e:
                return JsonResponse({'error': str(e)}, status=400)
        else:
            return JsonResponse(form.errors, status=400)

def data_visualization(request, summoner_name):
    summ_info = request.session.get('summ_info', None)
    
    if summ_info is None or summ_info[0] != summoner_name:
        return redirect('home')
    
    summoner_name, summoner_tag, summoner_region = summ_info

    api_key = settings.RIOT_API_KEY
    cache_key = f"{summoner_name}-{summoner_tag}-{summoner_region}"
    context = cache.get(cache_key)

    if not context:
        try:
            summoner = Summoner.objects.get(summoner_name=summoner_name, summoner_tag=summoner_tag, region=summoner_region)
            existing_match_ids = set(Match.objects.filter(summoner_id=summoner).values_list('id', flat=True))
            api_match_ids = get_match_list(summoner.puuid, summoner_region, api_key) 

            new_match_ids = [mid for mid in api_match_ids if mid not in existing_match_ids]

            if new_match_ids:
                save_matches_stats(summoner, api_key) 
